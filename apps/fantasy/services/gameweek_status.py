...
from apps.kpl.models import Gameweek, Fixture
from apps.kpl.models import FixtureLineup
...
class GameweekStatusService:
    ...
    def get_comprehensive_gameweek_status(self, gameweek_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        return {
            "gameweek": {
                ...
                "progress_percentage": self._calculate_progress_percentage(gameweek, fixtures)
            },
    ...
    def _get_gameweek_fixtures(self, gameweek: Gameweek) -> List[Fixture]:
        ...
        return list(Fixture.objects.filter(gameweek=gameweek).select_related(
            'home_team', 'away_team'
    ...
    def _get_fixtures_summary(self, fixtures: List[Fixture]) -> Dict[str, Any]:
        ...
        home_wins = len([f for f in completed_fixtures if (f.home_team_score or 0) > (f.away_team_score or 0)])
        draws = len([f for f in completed_fixtures if f.home_team_score == f.away_team_score])
        away_wins = len([f for f in completed_fixtures if (f.away_team_score or 0) > (f.home_team_score or 0)])
        ...
        return {
            ...
            "average_goals": round(total_goals / len(completed_fixtures), 2) if completed_fixtures else 0,
            "home_wins": home_wins,
            "draws": draws,
            "away_wins": away_wins,
    ...
    def _get_team_performance_summary(self, fixtures: List[Fixture]) -> Dict[str, Any]:
        ...
        completed_fixtures = [f for f in fixtures if f.status == "completed"]
        team_stats = defaultdict(lambda: {"played": 0, "goals_for": 0, "goals_against": 0, "points": 0})
        ...
        best_attack = max(team_stats.items(), key=lambda x: x[1]["goals_for"]) if team_stats else None
        best_defense = min(team_stats.items(), key=lambda x: x[1]["goals_against"]) if team_stats else None
        
        return {
            "teams_played": len([team for team, stats in team_stats.items() if stats["played"] > 0]),
            "best_attack": {"team": best_attack[0], "goals": best_attack[1]["goals_for"]} if best_attack else None,
            "best_defense": {"team": best_defense[0], "goals_conceded": best_defense[1]["goals_against"]} if best_defense else None,
            "team_stats": dict(team_stats)
        }
    
    def _get_interesting_stats(self, gameweek: Gameweek, fixtures: List[Fixture]) -> List[Dict[str, Any]]:
        """Generate interesting statistics and facts"""
        stats = []
        
        # Goals statistics
        completed_fixtures = [f for f in fixtures if f.status == "completed"]
        total_goals = sum((f.home_team_score or 0) + (f.away_team_score or 0) for f in completed_fixtures)
    ...
    def _get_lineups_status(self, fixtures: List[Fixture]) -> Dict[str, Any]:
        ...
        for fixture in fixtures:
            lineups = FixtureLineup.objects.filter(fixture=fixture).select_related('team')
            ...
            lineup_data.append({
                ...
                "home_lineup_confirmed": home_lineup.is_confirmed if home_lineup else False,
                "away_lineup_confirmed": away_lineup.is_confirmed if away_lineup else False,
        ...
        return {
            ...
            "confirmation_percentage": int((confirmed_lineups / len(fixtures)) * 100) if fixtures else 0,
    ...
    def _get_highest_scoring_match(self, fixtures: List[Fixture]) -> Optional[Dict[str, Any]]:
    ...
    def _get_day_interesting_fact(self, day_fixtures: List[Fixture]) -> str:
        ...
        total_goals = sum((f.home_team_score or 0) + (f.away_team_score or 0) for f in completed)
    ...
    def _compare_to_previous_gameweek(self, current_gameweek: Gameweek, stat_type: str) -> str:
