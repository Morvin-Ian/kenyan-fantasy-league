...
class FantasyPointsCalculator:
    ...
    def calculate_incremental(new_performance, old_values, is_captain=False):
        ...
        penalties_saved_diff = new_performance.penalties_saved - old_values['penalties_saved']
        if penalties_saved_diff > 0:
            points += penalties_saved_diff * 5

        penalties_missed_diff = new_performance.penalties_missed - old_values['penalties_missed']
...
class MatchEventService:
        ...
        with transaction.atomic():
            for assist_data in assists_data:
                ...
                if not player_name or not team_id:
                    errors.append({"player_name": player_name, "error": "player_name and team_id are required"})
                    continue
                ...
                try:
                    ...
                    if not player:
                        errors.append({"player_name": player_name, "error": f"Player not found in team {team.name}"})
                        continue
                    ...
                    performance, created = PlayerPerformance.objects.get_or_create(
                        ...
                        gameweek=fixture.gameweek,
                        defaults={
                            'assists': 0, 'goals_scored': 0, 'minutes_played': 0,
                            'yellow_cards': 0, 'red_cards': 0, 'clean_sheets': 0,
                            'saves': 0, 'own_goals': 0, 'penalties_saved': 0,
                            'penalties_missed': 0, 'fantasy_points': 0
                        }
                    )
                    ...
                    if created:
                        performance.fantasy_points = FantasyPointsCalculator.calculate_full(performance, is_captain)
                    else:
                        incremental_points = FantasyPointsCalculator.calculate_incremental(performance, old_values, is_captain)
                    ...
                    errors.append({"player_name": player_name, "error": f"Team with id {team_id} not found"})
                except Exception as e:
    ...
    def update_cards(fixture: Fixture, yellow_cards: List[Dict], red_cards: List[Dict]) -> Dict:
        ...
        with transaction.atomic():
            for card_data in yellow_cards:
                result = MatchEventService._process_card(fixture, card_data, "yellow_cards", "yellow")
            ...
            for card_data in red_cards:
                result = MatchEventService._process_card(fixture, card_data, "red_cards", "red")
                if result.get("success"):
                    updated_players.append(result["data"])
                else:
                    errors.append(result["error"])
            ...
            for sub_data in substitutions:
                ...
                try:
                    ...
                    if player_out:
                        ...
                        if created_out:
                            perf_out.fantasy_points = FantasyPointsCalculator.calculate_full(perf_out, is_captain_out)
                        else:
                            incremental_points = FantasyPointsCalculator.calculate_incremental(perf_out, old_values_out, is_captain_out)
                        ...
                        errors.append({"player_name": player_out_name, "error": f"Player not found in team {team.name}"})
                    ...
                    if player_in:
                        ...
                        if created_in:
                            perf_in.fantasy_points = FantasyPointsCalculator.calculate_full(perf_in, is_captain_in)
                        else:
                            incremental_points = FantasyPointsCalculator.calculate_incremental(perf_in, old_values_in, is_captain_in)
                        ...
                        errors.append({"player_name": player_in_name, "error": f"Player not found in team {team.name}"})
    ...
    @staticmethod
    def update_goalkeeper_stats(fixture: Fixture, saves: List[Dict], penalties_saved: List[Dict], penalties_missed: List[Dict]) -> Dict:
        ...
        with transaction.atomic():
            ...
            for stat_data in penalties_saved:
                result = MatchEventService._process_stat(fixture, stat_data, "penalties_saved")
                if result.get("success"):
                    updated_players.append(result["data"])
                else:
                    errors.append(result["error"])
            ...
            for stat_data in penalties_missed:
                result = MatchEventService._process_stat(fixture, stat_data, "penalties_missed")
                if result.get("success"):
                    updated_players.append(result["data"])
                else:
                    errors.append(result["error"])
    ...
    def _process_card(fixture, card_data, field_name, card_type):
        ...
        if not player_name or not team_id:
            return {
                "success": False,
                "error": {"player_name": player_name, "error": "player_name and team_id are required"},
            }
        ...
        try:
            ...
            if not player:
                return {
                    "success": False,
                    "error": {"player_name": player_name, "error": f"Player not found in team {team.name}"},
                }
            ...
            return {
                "success": False,
                "error": {"player_name": player_name, "error": f"Team with id {team_id} not found"},
            }
        except Exception as e:
            return {
                "success": False,
