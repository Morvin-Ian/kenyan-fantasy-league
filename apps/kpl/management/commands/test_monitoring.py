...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--setup", action="store_true", help="Setup monitoring tasks"
        )
        parser.add_argument(
            "--list", action="store_true", help="List all monitoring tasks"
        )
        parser.add_argument(
            "--test-fixture", type=str, help="Test specific fixture by ID"
        )
        parser.add_argument(
            "--cleanup", action="store_true", help="Clean up all monitoring tasks"
        )
    ...
    def list_monitoring_tasks(self):
        ...
        for task in tasks:
            ...
            self.stdout.write(
                f"  - {task.name}: {status} (Last run: {task.last_run_at})"
            )
    ...
    def show_status(self):
        ...
        if active_gameweek:
            ...
            fixtures = Fixture.objects.filter(
                gameweek=active_gameweek, status__in=["upcoming", "live"]
            )
            self.stdout.write(f"Fixtures in active gameweek: {fixtures.count()}")

            for fixture in fixtures:
                self.stdout.write(
                    f"  - {fixture.home_team} vs {fixture.away_team} ({fixture.status})"
                )
        else:
