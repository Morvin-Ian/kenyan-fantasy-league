...
def parse_scorers_page(html_content: str) -> list[dict]:
    ...
    player_links = [
        link for link in all_links if "player_goals.php" in link.get("href", "")
    ]
    team_links = [
        link for link in all_links if "team_goals.php" in link.get("href", "")
    ]

    logger.info(
        f"Found {len(player_links)} player links and {len(team_links)} team links"
    )
    ...
    while i < len(lines) and len(scorers) < 20:
        ...
        if line.isdigit() and 1 <= int(line) <= 100:
            ...
            if i + 1 < len(lines):
                ...
                if (
                    potential_player in player_names
                    and potential_player not in seen_players
                ):
                        ...
                        if potential_team in team_names:
                            ...
                            if i + 3 < len(lines):
                                ...
                                if potential_goals.isdigit():
                                    ...
                                    logger.debug(
                                        f"Found scorer: {player_name} ({team_name}) - {goals} goals"
                                    )
    ...
    if len(scorers) < 10:
        logger.warning(
            f"Only found {len(scorers)} scorers with strict parsing, trying lenient approach"
        )
        scorers = []
        seen_players = set()
        ...
        for idx, player_name in enumerate(player_names[:20]):
            ...
            if player_index != -1:
                ...
                if team_index != -1:
                    ...
                    chunk = text_content[
                        team_index + len(team_name) : team_index + len(team_name) + 50
                    ]
...
def scrape_top_scorers(gameweek_id=None):
    ...
    try:
        ...
        for scorer_data in scorers:
            ...
            player = find_player(
                scorer_data["player_name"], team_name=scorer_data["team_name"]
            )
        ...
        logger.info(
            f"Successfully saved {saved_count} scorers for Gameweek {gameweek.number}"
        )
        return {"success": True, "gameweek": gameweek.number, "count": saved_count}
