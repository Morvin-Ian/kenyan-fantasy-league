    ...
    if web_content.status_code == 200:
        ...
        for row in table_rows:
            ...
            try:
                ...
                logger.error(f"Could not split team names from match text: '{match_text}'")
                continue

            home_team = find_team(home_team_name.strip())
            away_team = find_team(away_team_name.strip())
            if not home_team or not away_team:
                logger.error(f"Skipping fixture: No team found for home='{home_team_name}' or away='{away_team_name}'")
                continue
            ...
            try:
                ...
                if existing_fixture:
                    if existing_fixture.match_date != match_datetime:
                        ...
                        fixtures_updated += 1
                        logger.info(f"Updated fixture: {home_team_name} vs {away_team_name} on {match_datetime}")
                else:
                    ...
                    fixtures_created += 1
                    logger.info(f"Created new fixture: {home_team_name} vs {away_team_name} on {match_datetime}")
        ...
        logger.error(f"Failed to retrieve the web page. Status code: {web_content.status_code}")
