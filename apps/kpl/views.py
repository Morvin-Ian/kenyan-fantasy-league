    ...
    def get_queryset(self):
        ...
        if getattr(self, "action", None) == "list":
            ...
            if active_gw_number is None:
                try:
                    active_gw = (
                        Gameweek.objects.filter(is_active=True).values("number").first()
                    )
            ...
            qs = (
                qs.filter(
                    gameweek__number__gte=max(
                        1, active_gw_number - 2
                    ),  # Current + last 2 GWs
    ...
    def lineups(self, request, id=None):
        ...
        qs = (
            ...
            .prefetch_related(
                Prefetch(
                    "players",
                    queryset=FixtureLineupPlayer.objects.select_related(
                        "player__team"
                    ).only(
                        "id",
                        "position",
    ...
    def submit_lineup(self, request):
        ...
        try:
            result = LineupService.submit_manual_lineup(
                serializer.validated_data, auto_update_performance
            )
...
class PlayerViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        return (
            Player.objects.filter(team__is_relegated=False)
            .select_related("team")
            .only(
                "id",
                "name",
