    ...
    def get(self, request):
        ...
        if state:
            try:
                ...
                logger.info(f"Parsed state - redirect_to: {redirect_to}, origin: {origin}")
        ...
        if not code:
            ...
            error_url = f"{redirect_to}?auth_success=false&auth_message=no_code&origin={origin}"
            return redirect(error_url)
        
        try:
            # Exchange code for tokens
            callback_uri = f"{settings.BASE_BACKEND_URL}/api/v1/auth/google/callback/"
            token_response = GoogleOAuthService.exchange_code_for_tokens(code, callback_uri)
            ...
            # Encode data for URL
            encoded_tokens = urllib.parse.quote(json.dumps({
                "access": tokens["access"],
                "refresh": tokens["refresh"],
            }))
            ...
            logger.info(f"Google OAuth successful for user: {user.email}, redirecting to: {redirect_to}")
