...
def test_nginx_reloads_mounted_host_rejection_configuration_promptly():
    """A corrected mounted config must not wait six hours to protect Django."""
    compose = (
        pathlib.Path(__file__).resolve().parents[1] / "docker-compose.prod.yml"
    ).read_text()

    assert "sleep 5m" in compose
    assert "nginx -t && nginx -s reload" in compose
