<template>
  <div class="relative w-18 sm:w-22 md:w-18 lg:w-20 xl:w-24 transition-all duration-300">
    <div
      ...
      :class="{ 'active': isActive, 'ring-2 ring-yellow-400': player.isCaptain || player.isViceCaptain }"
    >
      ...
      <div class="relative mb-2 flex justify-center items-center">
        ...
        <div class="jersey-container relative group">
          ...
          />
          <div class="absolute inset-0 bg-green-500/10 rounded-full transform scale-110 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
        </div>
      ...
      <div class="points-container relative mt-1 bg-gradient-to-b from-green-800/60 to-green-900/80 rounded-full py-1 px-1 backdrop-blur-sm border border-green-500/20">
        <div class="shine-effect absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300"></div>
        <p class="text-white text-xs sm:text-sm font-bold leading-tight tracking-wide">
          {{ player.points }} <span class="text-green-300 text-[0.6rem]">PTS</span>
        </p>
      </div>

      <div v-if="isActive" class="absolute -top-2 -right-2 w-3 h-3 bg-green-400 rounded-full shadow-md border border-white"></div>
    </div>
  </div>
</template>
...
.points-container:hover {
  background: linear-gradient(to bottom, rgba(6, 95, 70, 0.7), rgba(5, 150, 105, 0.8));
