  ...
  <div
    ...
    <div class="text-center mb-4">
      <div class="flex items-center justify-center gap-2 mb-2">
        <div class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium" :class="{
          ...
          <svg v-if="match.status === 'live'" class="w-3 h-3 mr-1 animate-pulse" fill="currentColor"
            viewBox="0 0 24 24">
            <circle cx="12" cy="12" r="8" />
          </svg>
          <svg v-else-if="match.status === 'postponed'" class="w-3 h-3 mr-1" fill="none" stroke="currentColor"
            viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
          </svg>
          <svg v-else-if="match.status === 'completed'" class="w-3 h-3 mr-1" fill="none" stroke="currentColor"
            viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center space-x-3 flex-1 min-w-0">
        <img :src="homeTeamLogo.logoUrl.value" :alt="match.home_team.name"
          class="w-12 h-12 rounded-full object-cover border-2 border-gray-100" @error="homeTeamLogo.handleError" />
      ...
      <div class="flex items-center space-x-3 flex-1 min-w-0 justify-end">
        ...
        <img :src="awayTeamLogo.logoUrl.value" :alt="match.away_team.name"
          class="w-12 h-12 rounded-full object-cover border-2 border-gray-100" @error="awayTeamLogo.handleError" />
      </div>
    </div>
    ...
    <div class="flex flex-wrap gap-2">
      <!-- View Lineup -->
      <button v-if="hasLineups" @click="emit('view-lineup', match)"
      ...
      <!-- Upload Lineup -->
      <button v-if="(match.status !== 'postponed') && isAdmin" @click="emit('open-upload', match)"
      ...
      <!-- Match Events (Admin) -->
      <button v-if="(match.status === 'completed' || match.status === 'live') && isAdmin"
      ...
      <!-- View Events -->
      <button v-if="(match.status === 'completed' || match.status === 'live') && hasEvents"
