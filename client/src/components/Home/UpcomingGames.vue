<template>
  <div class="bg-gradient-to-br from-gray-50 to-blue-50 p-4 sm:p-6 md:p-8 rounded-2xl mx-2 sm:mx-4 shadow-lg mb-3">
    ...
    <!-- Matches Container -->
    <div v-if="activeStandings.length > 0" ref="scrollContainer"
    ...
    <div v-else
      ...
      <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 mb-3 text-gray-400" fill="none" viewBox="0 0 24 24"
    ...
    <div class="animate-fade-in mb-6 flex flex-col sm:flex-row sm:justify-between items-start sm:items-center mt-8">
      <div>
        <h2 class="text-xl sm:text-2xl font-bold text-gray-700 bg-clip-text">
          League Standings
        </h2>
        ...
        <table class="w-full min-w-full table-auto">
          <thead>
            <tr class="border-b border-gray-100">
              ...
              <th v-for="col in ['MP', 'W', 'D', 'L', 'GF', 'GA', 'GD', 'PTS']" :key="col"
          ...
          <tbody>
            <tr v-for="(team, index) in firstFiveStandings" :key="team.id"
              ...
              <td
                ...
                :key="value" class="py-3 md:py-4 px-2 md:px-4 text-center text-gray-700 text-xs sm:text-sm font-medium">
                {{ value }}
              </td>
            </tr>
          </tbody>
...
const getTeamLogoUrl = (teamId: string): string => {
  if (logoCache.value.has(teamId)) {
    return logoCache.value.get(teamId)!;
  }
...
const handleLogoError = (event: Event, teamId: string) => {
  const img = event.target as HTMLImageElement;
  const standingsLogo = getTeamLogoUrl(teamId);

  if (standingsLogo && img.src !== standingsLogo) {
    img.src = standingsLogo;
  } else if (!standingsLogo && img.src !== defaultLogo) {
    img.src = defaultLogo;
  }
};
</script>
