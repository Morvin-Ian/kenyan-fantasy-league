<template>
    <div class="bg-gradient-to-br from-gray-50 via-gray-100 to-gray-50 p-8 sm:p-10 rounded-3xl mx-auto shadow-2xl max-w-7xl mb-8">
      <div class="bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl overflow-hidden border border-indigo-100">
        <!-- Header -->
        <div class="relative p-8 bg-gradient-to-r from-blue-900 to-indigo-800 overflow-hidden">
          ...
          <h1 class="text-3xl font-bold text-white relative z-10 flex items-center">
            ...
            <span class="ml-4 text-sm bg-indigo-600 px-3 py-1 rounded-full">Pro Version</span>
          </h1>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead class="bg-gradient-to-r from-blue-100 to-indigo-100">
              <tr>
                <th class="px-8 py-4 text-left text-blue-800 font-semibold">Player</th>
                <th class="px-8 py-4 text-left text-blue-800 font-semibold">Position</th>
                <th class="px-8 py-4 text-left text-blue-800 font-semibold">UCI Ranking</th>
                <th class="px-8 py-4 text-left text-blue-800 font-semibold">Form</th>
                <th class="px-8 py-4 text-left text-blue-800 font-semibold">Fantasy Points</th>
                <th class="px-8 py-4 text-left text-blue-800 font-semibold">Action</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                  ...
                  <div class="flex items-center">
                    <div class="relative h-16 w-16 mr-4">
                      <img
                        :src="player.image"
                        :alt="player.name"
                        class="h-16 w-16 rounded-full object-cover shadow-lg transition-all duration-300 hover:scale-105 ring-2 ring-offset-2"
                        :class="{
                      ...
                      />
                      <div v-if="player.uciRanking <= 3" class="absolute -top-2 -right-2 bg-yellow-400 text-white rounded-full h-6 w-6 flex items-center justify-center text-xs font-bold shadow-md">
                        {{ player.uciRanking }}
                      </div>
                    </div>
                    <div>
                ...
                <td class="px-8 py-6">
                  <span
                    class="px-4 py-2 rounded-full text-xs font-semibold shadow-sm"
                    :class="{
                ...
                <td class="px-8 py-6">
                  <div class="flex items-center">
                    <div class="w-32 bg-gray-200 rounded-full h-2.5 mr-2">
                      <div class="h-2.5 rounded-full" :style="{ width: `${player.form}%` }" :class="{
                        'bg-green-500': player.form >= 80,
                        'bg-yellow-500': player.form >= 60 && player.form < 80,
                        'bg-red-500': player.form < 60
                      }"></div>
                    </div>
                    <div class="text-gray-700">{{ player.form }}%</div>
                  </div>
                </td>
                <td class="px-8 py-6">
                ...
                <td class="px-8 py-6">
                  <button
                    ...
                    class="px-6 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg transition-all duration-300 hover:shadow-lg hover:from-blue-600 hover:to-indigo-700 transform hover:-translate-y-1 flex items-center"
                  >
        ...
        <!-- Pagination -->
        <div class="flex justify-between items-center p-8 bg-gradient-to-b from-white to-blue-50 border-t border-blue-100">
          ...
          <div class="flex space-x-2">
            <button
              ...
              class="px-3 py-2 rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-blue-700"
            ...
            <button
              ...
              :disabled="currentPage === 1"
              class="px-6 py-2 bg-blue-100 text-blue-700 rounded-lg transition-all duration-300 hover:bg-blue-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
            >
  ...
  // Enhanced player data with more variety
  ...
  const avgRating = computed(() => {
    ...
    const total = filteredPlayers.value.reduce((sum, player) => sum + player.form, 0);
    return Math.round(total / filteredPlayers.value.length);
  });
  
  const topPosition = computed(() => {
    if (filteredPlayers.value.length === 0) return "N/A";
    const positionCount = {};
    ...
    return positionMap[topPos] || topPos;
  });
  
  const bestPlayer = computed(() => {
    if (filteredPlayers.value.length === 0) return "N/A";
    const best = filteredPlayers.value.reduce((prev, current) => 
  ...
  <style scoped>
  .stats-card {
    @apply bg-white p-4 rounded-xl shadow-sm border border-blue-100 flex-1 min-w-32;
  }
