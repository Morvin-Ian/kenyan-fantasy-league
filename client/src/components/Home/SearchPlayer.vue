  ...
  <div class="bg-gradient-to-br from-gray-50 via-gray-100 to-gray-50 p-4 sm:p-8 rounded-3xl mx-4 shadow-2xl mb-8">
    <div class="bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl overflow-hidden border border-indigo-100">
      ...
      <div class="relative p-6 sm:p-8 bg-gradient-to-r from-blue-900 to-indigo-800 overflow-hidden">
        ...
        <p class="text-blue-100 mt-2 max-w-2xl text-sm sm:text-base">Find and add the best players to your fantasy team.
          Search, filter, and
          discover top talent from around the world.</p>
      </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <div class="relative group transform transition-all duration-300 hover:scale-105">
            <select v-model="filters.team"
              ...
              <option v-for="team in kplStore.teams" :key="team.id" :value="team.name">
                {{ team.name }}
              </option>
            </select>
            <svg xmlns="http://www.w3.org/2000/svg"
      ...
      <!-- Player Table with improved design -->
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="(player, index) in paginatedPlayers" :key="player.id"
              class="border-b hover:bg-blue-50 transition-all duration-300" :class="{ 'animate-fadeIn': showAnimation }"
            ...
            <tr v-if="paginatedPlayers.length === 0">
              <td colspan="6" class="text-center py-16 text-gray-500 bg-blue-50/50">
                ...
                <button @click="clearFilters" class="text-blue-500 underline mt-2 hover:text-blue-700">Clear filters and
                  try again</button>
              </td>
            </tr>
          </tbody>
...
const getPositionName = (position: string): string => {
  ...
  return positionMap[validPosition] || position; // Fallback to the original value if not found
};
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(filteredPlayers.value.length / playersPerPage.value))
);
...
const paginatedPlayersEnd = computed(() => {
  return Math.min(
    currentPage.value * playersPerPage.value,
...
const addPlayer = (player: Player) => {
  ...
  alert(`Added ${player.name} (${position}) to your fantasy team!`);
};
...
// Adjust current page when total pages changes
...
// Reset to page 1 when players per page changes
